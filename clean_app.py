...
def main():
    ...
    page = st.sidebar.selectbox(
        "Choose a section",
        ["Predict Price", "Data Analysis", "API Testing", "Model Performance", "System Status", "About"]
    )
        ...
        with st.spinner("Making prediction..."):
            ...
            if not similar_properties.empty:
                for idx, prop in similar_properties.iterrows():
                    st.write(f"Location: {prop['Street']} - ${prop['ListedPrice']:,.2f} ({prop['Bedroom']} bed, {prop['Bathroom']} bath, {prop['Area']:,.0f} sq ft)")
    ...
    with col2:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                st.warning(f"Streamlit Web App: Status {response.status_code}")
        except:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                st.warning(f"Flask API: Status {response.status_code}")
        except:
...
def show_about_page():
    ...
    st.write("House Price Predictor - Complete Suite")
    st.write("")
